[![forthebadge made-with-python](http://ForTheBadge.com/images/badges/made-with-python.svg)](https://www.python.org/)
[![LICENSE](https://img.shields.io/badge/license-MIT-lightgrey.svg)](https://github.com/taapas1128/Calculator_pyqt/blob/master/LICENSE)

[![KWoC 2018](https://img.shields.io/badge/KWoC-2018-0078D6.svg?style=for-the-badge&longCache=true&logo=data:image/png;base64,)](https://kwoc.kossiitkgp.org/)

# PyQt Calculator
This is a basic PyQt based Scientific Calculator 

Instructions to Run on Your Machine:
* Before you can build PyQt4 you must have already built and installed [SIP](https://pypi.org/project/SIP/) .

* You need to install PyQt4 dependency which can be installed on Linux Platform as `sudo apt-get install python-qt4.`
For Windows Platform Download wheel file from [here](https://www.lfd.uci.edu/~gohlke/pythonlibs/#pyqt4) & Then locate the download file directory in Windows PowerShell & Run `pip install .\pyqt4-file-name.whl` . 
NOTE: 'pyqt-file-name' should be replaced by the name of wheel file you've downloaded!
![pyqt install](pyqt4inst.jpg)

* Run `logic.py` for running the calculator on Linux Platform & Run `python logic.py` in Windows PowerShell for Windows Platform.This contains the basic evaluation code and the logic behind the calculator.

* `calculator.ui` is the UI design used which can be created by PyQt Designer GUI. For UI modification you can install PyQt designer which can be installed as given [here](https://askubuntu.com/questions/763877/how-to-install-and-run-qt-designer-for-python) .

* `calculator.py` is extract code of the UI using PyQt.

* Any `input.ui` file can be converted to `output.py` by `pyuic4 input.ui -o output.py` for pyqt4 dependencies.

* You can also go through these set of videos for understanding the framework and functioning of qt designer. Click [here](https://www.youtube.com/watch?v=iDJlGV8Rvgg).

* Here is the Layout of Our Scientific Calculator!
  
  ![calculator](calc.jpg)
